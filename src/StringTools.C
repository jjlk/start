...
int StringToTelList (const std::string &s, std::vector<unsigned int> &tellist) {

  std::string valstring = s;
  ...
  if ( valstring != "" ) {
    std::vector<std::string> newvec = split(valstring,", \t");
    for (std::vector<std::string>::iterator inew = newvec.begin();
      ...
      if ( !(vstream >> val) ) {
	std::cout << "** ERROR: couldn't convert string to tellist." << std::endl;
	return 0;
      }
  ...
  //If the input string is a list, use it
  if (list.size() > 1)
    tellist = list;
  //If the input string is a single value, interpret it as a pattern
  else if (list.size()) {
    for(int telid = 1;telid<16;telid++){
      unsigned int telbit = 1 << telid;
      if ((telbit & list[0]) == telbit){
	tellist.push_back(telid);
      }
    }
  }
  
  return (int) tellist.size ();
}

int TelListToString (const std::vector<unsigned int> &tellist, std::string &s) {

  s = join( tellist, std::string(",") );
  return (int) tellist.size ();
