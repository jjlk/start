...
// START
#include "SuperExpoCutOffPowerLaw.hh"
...
ClassImp(START::SuperExpoCutOffPowerLaw)
#endif
 ...
 */
START::SuperExpoCutOffPowerLaw::SuperExpoCutOffPowerLaw()
  :Hypothesis()
{
 ...
 */
START::SuperExpoCutOffPowerLaw::SuperExpoCutOffPowerLaw(TString name, SpectralType type, double phi0, 
						 double gamma1, double gamma2, double beta, 
						 double phi0err, double gamma1err, double gamma2err,
						 double betaerr)
  :Hypothesis(name)
{
  ...
  fparamerr[0] = phi0err;
  fparamerr[1] = gamma1err;
  fparamerr[2] = gamma2err;
  fparamerr[3] = betaerr;

  InitParametersCaracteristics();

  fmodelname = "SuperExpoCutOffPowerLaw";

  flatexformula = "#left(#frac{E}{E_{Ref}} #right)^{-#Gamma_{1}} e^{(-#beta E)^{#Gamma_{2}}}";
  ...
  fFunctorFlux = new  ROOT::Math::Functor1D(this,&START::SuperExpoCutOffPowerLaw::FluxFormula);
  fFunctorFluxTimesEnergy = new  ROOT::Math::Functor1D(this,&START::SuperExpoCutOffPowerLaw::FluxTimesEnergy);
 ...
 */
START::SuperExpoCutOffPowerLaw* START::SuperExpoCutOffPowerLaw::clone() const
{
  return new SuperExpoCutOffPowerLaw(*this);
}
 ...
 */
START::SuperExpoCutOffPowerLaw::SuperExpoCutOffPowerLaw(SuperExpoCutOffPowerLaw const &StoreCopy)
 ...
 */
double START::SuperExpoCutOffPowerLaw::FluxFormula(double x) const {
  return TMath::Power((x/feref),-fparam[1])*TMath::Exp(-TMath::Power((x*fparam[3]),fparam[2]));
}
 ...
 */
double START::SuperExpoCutOffPowerLaw::FluxTimesEnergy(double x) const {
 ...
 */
double START::SuperExpoCutOffPowerLaw::GetFlux(double x) const
{
  double integrant;
 ...
 */
void START::SuperExpoCutOffPowerLaw::DerivativesFormulae(double x)
{
  
  double phi0(fparamfit[0]);
  double gamma1(fparamfit[1]);
  double gamma2(fparamfit[2]);
  double beta(fparamfit[3]);
  double betax(x*fparamfit[3]);
  double phi_log(TMath::Power((x/feref),-gamma1));
  double phi_exp(TMath::Exp(-TMath::Power(betax,gamma2)));

  ffirstderivatives[0] = phi_log*phi_exp; // dphi/dphi0
  ffirstderivatives[1] = -phi0*TMath::Log(x/feref)*phi_log*phi_exp; // dphi/dgamma1
  ffirstderivatives[2] = -phi0*TMath::Log(betax)*TMath::Power(betax,gamma2)*phi_log*phi_exp; // dphi/dgamma2
  ffirstderivatives[3] = -phi0*gamma2*TMath::Power(betax,gamma2)/beta*phi_log*phi_exp; // dphi/dbeta
 ...
 */
void START::SuperExpoCutOffPowerLaw::InitParametersCaracteristics() {
  ...
  case Differential:
    ...
    flatexparamname[1] = "#Gamma_{1}";
    flatexparamname[2] = "#Gamma_{2}";
    flatexparamname[3] = "#beta";
