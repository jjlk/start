...
ClassImp(START::Sum3Hypothesis)
#endif

/**
 * \brief Default constructor needed by root
 */
START::Sum3Hypothesis::Sum3Hypothesis()
  :Hypothesis()
{
 ...
 */
START::Sum3Hypothesis::Sum3Hypothesis(TString name, const Hypothesis &hypo1, const Hypothesis &hypo2,const Hypothesis &hypo3, bool comparison)
  :Hypothesis(name)
{
  ...
  if(fiscomparison) {
    ...
    fnumberofparameters=fHypo1->GetParametersNb()+fHypo2->GetParametersNb()+fHypo3->GetParametersNb();
  }
  ...
  if(fiscomparison) {
    ...
    for(unsigned int ipar(0); ipar<fHypo3->GetParametersNb(); ipar++) {
      fparam.push_back(fHypo3->GetParameters()[ipar]);
      fparamerr.push_back(fHypo3->GetParametersErrors()[ipar]);
    }
    ...
    std::vector<std::pair<unsigned int,double> > normparam3 = fHypo3->GetVectorNormalizedParameters();
    ...
    for(unsigned int inorm(0); inorm<normparam3.size(); inorm++) {
      SetLowerLimitOnNormalizationParameter(fHypo1->GetParametersNb()+fHypo2->GetParametersNb()+normparam3[inorm].first,normparam3[inorm].second);
    }
    ...
    std::vector<std::pair<unsigned int, std::pair<double, bool> > > fixedparam3 = fHypo3->GetFixedParameters();
    ...
    for(unsigned int ifix(0); ifix<fixedparam3.size(); ifix++) {
      FixParameter(fixedparam3[ifix].first,fixedparam3[ifix].second.first,fixedparam3[ifix].second.second);
    }
  ...
  flatexformula+=" + ";
  flatexformula = fHypo3->GetLatexFormula();

  fmodelname = fHypo1->GetModelName();
  fmodelname = " + ";
  fmodelname = fHypo2->GetModelName();
  fmodelname = " + ";
  fmodelname = fHypo3->GetModelName();
}
 ...
 */
START::Sum3Hypothesis::Sum3Hypothesis(Sum3Hypothesis const &StoreCopy)
 ...
 */
START::Sum3Hypothesis* START::Sum3Hypothesis::clone() const
{
  return new Sum3Hypothesis(*this);
}

/**
 * \brief Return the flux for a given energy x used for Minuit2's callings
 */
double START::Sum3Hypothesis::GetFlux(double x) const
{
  if(fiscomparison) {
    ...
    for(unsigned int ipar(fHypo1->GetParametersNb()); ipar<(fHypo1->GetParametersNb()+fHypo2->GetParametersNb()); ipar++) paramhypo2.push_back(fparam[ipar]);
    fHypo2->SetParameters(paramhypo2);

    std::vector<double> paramhypo3;
    for(unsigned int ipar(fHypo1->GetParametersNb()+fHypo2->GetParametersNb()); ipar<(fHypo1->GetParametersNb()+fHypo2->GetParametersNb()+fHypo3->GetParametersNb()); ipar++) paramhypo3.push_back(fparam[ipar]);
    fHypo2->SetParameters(paramhypo3);

    return (fHypo1->GetFlux(x)+fHypo2->GetFlux(x)+fHypo2->GetFlux(x));
  }
}
 ...
 * \param x energy
 */
void START::Sum3Hypothesis::DerivativesFormulae(double x)
{
  ...
  if(!fiscomparison) {
    ...
    for(unsigned int ipar(0); ipar<fHypo1->GetParametersNb(); ipar++)  paramfithypo1.push_back(fparamfit[ipar]);
    fHypo1->SetFittedParameters(paramfithypo1);


    std::vector<double> paramfithypo2;
    for(unsigned int ipar(fHypo1->GetParametersNb()); ipar<(fHypo2->GetParametersNb()+fHypo1->GetParametersNb()); ipar++) paramfithypo2.push_back(fparamfit[ipar]);
    fHypo2->SetFittedParameters(paramfithypo2);


    std::vector<double> paramfithypo3;
    for(unsigned int ipar(fHypo1->GetParametersNb()+fHypo2->GetParametersNb()); ipar<(fHypo1->GetParametersNb()+fHypo2->GetParametersNb()+fHypo3->GetParametersNb()); ipar++) paramfithypo3.push_back(fparamfit[ipar]);
    fHypo3->SetFittedParameters(paramfithypo3);

    fHypo1->DerivativesFormulae(x);
    fHypo2->DerivativesFormulae(x);
    fHypo3->DerivativesFormulae(x);

    for(unsigned int ipar(0); ipar<fHypo1->GetParametersNb(); ipar++) {
      ffirstderivatives.push_back(fHypo1->GetFirstDerivatives()[ipar]);
    }
    //Test AL -> search the bug !!
    for(unsigned int ipar(0); ipar<fHypo2->GetParametersNb(); ipar++) {
      ffirstderivatives.push_back(fHypo2->GetFirstDerivatives()[ipar]);
    }
    for(unsigned int ipar(0); ipar<fHypo3->GetParametersNb(); ipar++) {
      ffirstderivatives.push_back(fHypo3->GetFirstDerivatives()[ipar]);
    }
 ...
 */
void START::Sum3Hypothesis::InitParametersCaracteristics()
{
  ...
  if(fiscomparison) {
    ...
    for(unsigned int ipar(0); ipar<fHypo1->GetParametersNb(); ipar++) {
      ...
      std::cout << "hypo1 : parameter name = " << name << std::endl;
    ...
    for(unsigned int ipar(0); ipar<fHypo2->GetParametersNb(); ipar++) {
      ...
      fparamname.push_back(name);
      flatexparamname.push_back(latexname);
      std::cout << "hypo2 : parameter name = " << name << std::endl;
      fparamunits.push_back(units);
    }
    for(unsigned int ipar(0); ipar<fHypo3->GetParametersNb(); ipar++) {
      std::string name(fHypo3->GetParametersNames()[ipar]), latexname(fHypo3->GetLatexParametersNames()[ipar]), units(fHypo3->GetParametersUnits()[ipar]);
      name+="_";
      name+=fHypo3->GetName();
      fparamname.push_back(name);
      flatexparamname.push_back(latexname);
      std::cout << "hypo3 : parameter name = " << name << std::endl;
      fparamunits.push_back(units);
    }
