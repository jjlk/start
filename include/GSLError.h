...
namespace ROOT { 
   ...
   namespace Math { 
	...
	GSLError() { 
	  ...
	  gsl_set_error_handler_off();//&GSLError::Handler); // JLK : turn off message errors from GSL // JLK ça ne marche plus...
	...
	static void Handler(const char * reason, const char * file, int line, int gsl_errno)  { 
	  ...
	  //Error("JLKGSLError","Error %d in %s at %d : %s",gsl_errno,file,line,reason); // JLK obliger de hacker ça...
